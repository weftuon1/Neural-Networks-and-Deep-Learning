...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    var = np.var(x, axis = 0)
    normalize_x = (x - mean) / np.sqrt(var + eps)
    ...
    cache = {'normalize_x': normalize_x,
             'x_minus_mean': (x - mean),
             'sqrt_var_eps': np.sqrt(var + eps),         
             'gamma': gamma      
            } 
...
def batchnorm_backward(dout, cache):
  ...
  normalize_x = cache.get('normalize_x')
  x_minus_mean = cache.get('x_minus_mean')
  sqrt_var_eps = cache.get('sqrt_var_eps')
  ...
  dnormalize_x = dout * gamma
    
  db = x_minus_mean * dnormalize_x # b = 1 / sqrt_var_eps
  dc = (-1 / (sqrt_var_eps * sqrt_var_eps)) * db # c = sqrt_var_eps
  de = (1 / (2 * sqrt_var_eps)) * dc # e = sqrt_var_eps * sqrt_var_eps
  dvar = np.sum(de, axis = 0)

  da = dnormalize_x / sqrt_var_eps # a = x - mu
  dmu = -np.sum(da, axis = 0) - 2 * np.sum(x_minus_mean, axis = 0) * dvar / N

  dx = da + 2 * x_minus_mean * dvar / N + dmu / N  
