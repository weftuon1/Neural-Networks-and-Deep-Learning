...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    probs_row = probs[range(N), y]
    probs_log = -np.log(probs_row)
    softmax_loss = np.sum(probs_log) / N
    ...
    dLdb = probs / N
    dLdW2 = np.maximum(np.dot(W1, X.T)+b1.reshape([W1.shape[0], 1]), 0)
    
    grads['W2'] = np.dot(dLdb.T, dLdW2.T) + reg * W2
    grads['b2'] = np.sum(dLdb, axis=0, keepdims=True)

    dbdh = W2.T 
    dLda = np.dot(dbdh, dLdb.T) * (np.dot(W1, X.T) > 0)
    
    grads['W1'] = np.dot(dLda, X) + reg * W1
  ...
  def predict(self, X):
    ...
    scores = np.dot(relu(h1), self.params['W2'].T) + self.params['b2']
